  ...
  return (
    <div
      className={`${bgColor} rounded-lg p-6 ${
        highlight ? "ring-4 ring-blue-500 shadow-xl" : "shadow-md"
      } hover:shadow-xl transition-shadow duration-200`}
    >
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-3">
        <h3 className={`font-bold text-2xl md:text-3xl ${accentColor}`}>{title}</h3>
        <span
          className={`inline-block mt-2 md:mt-0 px-4 py-2 ${bgColor.replace(
            "100",
            "200"
          )} rounded-full font-semibold ${accentColor}`}
        >
          {ageGroup}
        </span>
      </div>
...
function CoreValueCardComponent({
  icon,
  title,
  description
}: {
...
function StatCardComponent({ number, label }: { number: string; label: string }) {
  return (
    <div className="bg-white/20 backdrop-blur rounded-lg p-6 text-center">
      <div className="outfit-heading text-4xl md:text-5xl text-white mb-2">
        {number}
      </div>
...
export default memo(ProgramCardComponent);
export const CoreValueCard = memo(CoreValueCardComponent);
